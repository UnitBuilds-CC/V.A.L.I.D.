...
public class PropertyWeirGenerator : ISourceGenerator
{
    ...
    private string GenerateSource(INamedTypeSymbol symbol)
    {
        ...
        foreach (var prop in properties)
        {
            sb.AppendLine($"                case \"{prop.Name}\": type = typeof({GetTypeForTypeof(prop.Type)}); return true;");
        }
        sb.AppendLine("            }");
    ...
    private string GenerateTests(Compilation compilation, INamedTypeSymbol symbol)
    {
        ...
        var namespaceName = symbol.ContainingNamespace.ToDisplayString();
        var className = symbol.Name;
        // Use GetProperties() to include both [ValidProperty] and [ValidField] members
        var properties = GetProperties(symbol);
    ...
    private string GetTestValue(ITypeSymbol type)
    {
        ...
        if (IsValidObject(type)) return $"new {GetTypeForTypeof(type)}()";
    ...
    /// <summary>
    /// Strips nullable reference type annotation (e.g. "Foo?" → "Foo") for use in typeof/new expressions.
    /// Nullable value types (Nullable&lt;T&gt;) are left unchanged.
    /// </summary>
    private static string GetTypeForTypeof(ITypeSymbol type)
    {
        if (type is INamedTypeSymbol named && named.IsGenericType && 
            named.OriginalDefinition.SpecialType == SpecialType.System_Nullable_T)
        {
            return type.ToDisplayString(); // Keep Nullable<T> as-is
        }
        // For nullable reference types, use WithNullableAnnotation to strip the ?
        var stripped = type.WithNullableAnnotation(NullableAnnotation.NotAnnotated);
        return stripped.ToDisplayString();
    }
