...
|---|---|---|
| `BusinessBase<T>` | `IValidObject` / `ValidObjectBase` | Interface + base class with `ValidId` |
| `BusinessListBase<T,C>` | `ValidSlab<T>` | Slab-allocated collection with bitmask tracking (`ValidList<T>` is `[Obsolete]`) |
...
| `MarkDirty()` | Automatic | Property setter flips bitmask bit |
| `MarkClean()` | `ResetDirtyFlags(true)` | Explicit bitmask reset |
...
public partial class Invoice : ValidObjectBase
{
    ...
    // ValidId, bitmask tracking, validation — all auto-generated by source generator
    // invoice.ValidId → "valid_1" (unique, stable identity)
}
...
2. ☐ Inherit `ValidObjectBase` and use `[ValidObject]` + source generator
3. ☐ Replace `RegisterProperty<T>` + `GetProperty`/`SetProperty` with `partial` properties
...
6. ☐ Replace `BusinessListBase<T,C>` with `ValidSlab<T>` (or `ValidList<T>` — now `[Obsolete]`)
...
11. ☐ Replace `GetHashCode()` usage with `ValidId` for identity/JS bridge registration
